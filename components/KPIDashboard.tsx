             ...
             {todaysEvents.length === 0 ? (
               ...
               <div className="space-y-3">
                 {todaysEvents.map(evt => (
                   <div key={evt.id} className="flex gap-3 p-3 bg-blue-50/30 rounded-xl border border-blue-100/50 hover:border-blue-200 transition-colors">
                      <div className="flex flex-col items-center justify-center px-2 border-r border-blue-100 min-w-[50px]">
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <div className="flex items-center gap-2 mt-1">
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wider ring-1 ring-inset ${getCategoryColor(evt.category_id)}`}>
                              {getCategoryName(evt.category_id)}
                            </span>
                            <span className="text-[9px] text-slate-400 truncate">{evt.department}</span>
                         </div>
                      </div>
                   </div>
                 ))}
             ...
             {upcomingEvents.length === 0 ? (
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                 {upcomingEvents.map(evt => (
                   <div key={evt.id} className="p-3 bg-white border border-gray-100 rounded-xl hover:shadow-md hover:border-blue-100 transition-all group">
                      <div className="flex justify-between items-start mb-2">
                         <div className="flex items-center gap-1.5">
                            <div className={`w-2 h-2 rounded-full ${getCategoryColor(evt.category_id).split(' ')[0]}`}></div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase">{new Date(evt.start_date).toLocaleDateString(undefined, { weekday: 'short', day: 'numeric' })}</span>
                         </div>
                         <span className="text-[9px] font-bold text-slate-300 bg-slate-50 px-1.5 rounded">{evt.department}</span>
                      </div>
                      <h4 className="text-xs font-bold text-slate-800 line-clamp-1 group-hover:text-blue-600 transition-colors">{evt.title}</h4>
                      <p className="text-[10px] text-slate-500 mt-1 line-clamp-1">{evt.description || 'Sin descripción'}</p>
                   </div>
                 ))}
               </div>
             )}
